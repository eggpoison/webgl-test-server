import { COLLISION_BITS, DEFAULT_COLLISION_MASK, HitboxCollisionTypeConst, IEntityType, PlayerCauseOfDeath, Point, SettingsConst, randFloat } from "webgl-test-shared";
...
export function createRockSpikeProjectile(spawnPosition: Point, size: number, frozenYetiID: number): Entity {
   ...
   const hitbox = new CircularHitbox(rockSpikeProjectile.position.x, rockSpikeProjectile.position.y, ROCK_SPIKE_MASSES[size], 0, 0, HitboxCollisionTypeConst.soft, ROCK_SPIKE_HITBOX_SIZES[size], rockSpikeProjectile.getNextHitboxLocalID(), rockSpikeProjectile.rotation);
