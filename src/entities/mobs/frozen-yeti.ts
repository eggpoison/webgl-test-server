...
export function createFrozenYeti(position: Point): Entity {
   ...
   const bodyHitbox = new CircularHitbox(frozenYeti.position.x, frozenYeti.position.y, 4, 0, 0, HitboxCollisionTypeConst.soft, FROZEN_YETI_SIZE / 2, frozenYeti.getNextHitboxLocalID(), frozenYeti.rotation);
   frozenYeti.addHitbox(bodyHitbox);

   const headHitbox = new CircularHitbox(frozenYeti.position.x, frozenYeti.position.y, 0.8, 0, HEAD_DISTANCE, HitboxCollisionTypeConst.soft, HEAD_HITBOX_SIZE / 2, frozenYeti.getNextHitboxLocalID(), frozenYeti.rotation);
   frozenYeti.addHitbox(headHitbox);

   // Paw hitboxes
   for (let i = 0; i < 2; i++) {
      const pawDirection = PAW_RESTING_ANGLE * (i === 0 ? -1 : 1);
      const hitbox = new CircularHitbox(frozenYeti.position.x, frozenYeti.position.y, 0.6, PAW_OFFSET * Math.sin(pawDirection), PAW_OFFSET * Math.cos(pawDirection), HitboxCollisionTypeConst.soft, PAW_SIZE / 2, frozenYeti.getNextHitboxLocalID(), frozenYeti.rotation);
