...
class BerrySnowbush extends Entity {
   ...
   constructor(position: Point) {
      super(position, {
         health: new HealthComponent(BerrySnowbush.HEALTH, false)
      }, EntityTypeConst.berry_snowbush);
      
      this.rotation = 2 * Math.PI * Math.random();

      const hitbox = new CircularHitbox(this, 0, 0, BerrySnowbush.RADIUS);
   ...
   public tick(): void {
      ...
      if (this.numBerries < 5) {
         ...
         if (this.berryGrowTimer >= BerrySnowbush.BERRY_GROW_TIME) {
            this.berryGrowTimer = 0;
            this.growBerry();
         }
      }
   ...
   private dropBerry(): void {
      ...
      do {
         ...
         const spawnOffset = new Vector(BerrySnowbush.BERRY_DROP_OFFSET, spawnDirection).convertToPoint();
      ...
      droppedItem.velocity.x = BerrySnowbush.BERRY_DROP_VELOCITY * Math.sin(spawnDirection + velocityDirectionOffset);
      droppedItem.velocity.y = BerrySnowbush.BERRY_DROP_VELOCITY * Math.cos(spawnDirection + velocityDirectionOffset);
   }
}
